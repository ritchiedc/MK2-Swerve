...
public class TurningEncoder implements Sendable {
  ...
  private void init() {
    m_prefName = String.format("TurningEncoderOffset(%1d)", m_analogInput.getChannel());
  ...
  public double getAngleDeg() {
    double angle = (m_analogInput.getVoltage() * (360.0 / 5.0)) - m_angleOffset;
    if (angle > 180)
      angle -= 360;
    if (angle < -180)
      angle += 360;
  ...
  public void reset() {
    m_angleOffset = m_analogInput.getVoltage() * (360.0 / 5.0);
